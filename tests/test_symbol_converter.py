...
from easyeda2kicad.kicad.parameters_kicad_symbol import KicadVersion, KiPinStyle
...
def test_px_to_mil_basic() -> None:
    assert px_to_mil(10) == 100.0
    assert px_to_mil("5") == 50.0
    assert px_to_mil(0) == 0.0
...
def test_pin_style_inverted_clock() -> None:
    pins = convert_ee_pins([_make_pin(dot=True, clock=True)], BBOX, KicadVersion.v6)
    assert pins[0].style == KiPinStyle.inverted_clock


def test_pin_style_inverted() -> None:
    pins = convert_ee_pins([_make_pin(dot=True, clock=False)], BBOX, KicadVersion.v6)
    assert pins[0].style == KiPinStyle.inverted


def test_pin_style_clock() -> None:
    pins = convert_ee_pins([_make_pin(dot=False, clock=True)], BBOX, KicadVersion.v6)
...
def test_convert_ee_paths_mlz() -> None:
    ...
    result = convert_ee_paths([path], BBOX, KicadVersion.v6)
    assert len(result) == 1
...
def test_polyline_fill_closes_polygon() -> None:
    ...
    result = convert_ee_polylines([pl], BBOX, KicadVersion.v6)
    assert len(result) == 1
    assert result[0].is_closed
...
def test_arc_invalid_structure(caplog: pytest.LogCaptureFixture) -> None:
    ...
    with caplog.at_level(logging.ERROR):
        result = convert_ee_arcs([arc], BBOX, KicadVersion.v6)
    assert result == []
