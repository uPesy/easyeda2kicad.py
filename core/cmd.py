...
class cmd_interface():
    ...
    def __init__(self):
        ...
        if not os.path.isfile("output_lib/easyeda2kicad.lib"):
            with open(file="output_lib/easyeda2kicad.lib", mode="w+", encoding='utf-8') as my_lib:
                my_lib.write(
        ...
        self.parser.add_argument("--lcsc_id", help="LCSC id", required=True) # For devlopment
        self.parser.add_argument("--symbol", help="Get symbol of the id", required=False, action='store_true')
        self.parser.add_argument("--footprint", help="Get footprint of the id", required=False, action='store_true')
