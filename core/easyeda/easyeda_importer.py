...
class easyeda_symbol_importer():
    ...
    def extract_easyeda_data(self):
        new_ee_symbol = ee_symbol(
            info=ee_symbol_info(
                ...
                package=self.ee_data_info['package']                if 'package' in self.ee_data_info else None,
                manufacturer=self.ee_data_info['BOM_Manufacturer']   if 'BOM_Manufacturer' in self.ee_data_info else None,
                datasheet=self.ee_data['lcsc']['url']               if 'url' in self.ee_data['lcsc']  else None,
                lcsc_id=self.ee_data['lcsc']['number']                  if 'number' in self.ee_data['lcsc']  else None,
                jlc_id=self.ee_data_info['BOM_JLCPCB Part Class']   if 'BOM_JLCPCB Part Class' in self.ee_data_info else None
            ),
            bbox=ee_symbol_bbox(
        ...
        for line in self.ee_data['dataStr']['shape']:
            ...
            # Pour les rectangles
                ...
                new_ee_symbol.rectangles.append(self.tune_ee_rectangle(rect=ee_rectangle))
            # Pour les polylines
            elif designator == 'PL':
                ee_polyline = self.extract_easyeda_polyline(pin_data=line)
                new_ee_symbol.polylines.append(self.tune_ee_polyline(polyline=ee_polyline))
            # Pour les ellipse
            elif designator == 'E':
                ... # TODO
            # Pour les arcs
    ...
    def extract_easyeda_pin(self, pin_data:str):
        ...
        pin_settings = ee_symbol_pin_settings(**dict(zip(ee_symbol_pin_settings.__fields__, ee_segments[0][1:])))
        pin_dot = ee_symbol_pin_dot(
            dot_x=float(ee_segments[1][0]),
            dot_y=float(ee_segments[1][1])
        )
        pin_path = ee_symbol_pin_path(
            path=ee_segments[2][0],
            color=ee_segments[2][1]
        )
        pin_name = ee_symbol_pin_name(**dict(zip(ee_symbol_pin_name.__fields__, ee_segments[3][0:])))
    ...
    def extract_easyeda_rectangle(self, pin_data:str):
        ...
        return ee_symbol_rectangle(**dict(zip(ee_symbol_rectangle.__fields__, ee_segment[1:])))

    #---------------------------------------

    def extract_easyeda_polyline(self, pin_data:str):
        ee_segment= pin_data.split('~')
        return ee_symbol_polyline(**dict(zip(ee_symbol_polyline.__fields__, ee_segment[1:])))

    #---------------------------------------

    def tune_ee_pin(self, pin:ee_symbol_pin):
        pin.settings.rotation = pin.settings.rotation if pin.settings.rotation !='' else '0'
        pin.settings.type =  easyeda_pin_type(int(pin.settings.type)).name if pin.settings.type != '' else 'unspecified'
...
class easyeda_footprint_importer():
    ...
    def __init__(self, easyeda_cp_cad_data):
        ...
        self.ee_data_info = easyeda_cp_cad_data['packageDetail']['dataStr']['head']['c_para']
    ...
    def extract_easyeda_data(self):
        ...
        for line in self.ee_data_str['shape']:
            ...
            if ee_designator == 'PAD':
                ee_pad = ee_footprint_pad(**dict(zip(ee_footprint_pad.__fields__, ee_fields[:18])))
                new_ee_footprint.pads.append(ee_pad)
            elif ee_designator == "TRACK":
                ee_track = ee_footprint_track(**dict(zip(ee_footprint_track.__fields__, ee_fields)))
                new_ee_footprint.tracks.append(ee_track)
            elif ee_designator == "HOLE":
                ee_hole = ee_footprint_hole(**dict(zip(ee_footprint_hole.__fields__, ee_fields)))
                new_ee_footprint.holes.append(ee_hole)
            elif ee_designator == "CIRCLE":
                ee_circle = ee_footprint_circle(**dict(zip(ee_footprint_circle.__fields__, ee_fields)))
                new_ee_footprint.circles.append(ee_circle)
            elif ee_designator == "ARC":
                ee_arc = ee_footprint_arc(**dict(zip(ee_footprint_arc.__fields__, ee_fields)))
                new_ee_footprint.arcs.append(ee_arc)
            elif ee_designator == "RECT":
                ee_rectangle = ee_footprint_rectangle(**dict(zip(ee_footprint_rectangle.__fields__, ee_fields)))
                new_ee_footprint.rectangles.append(ee_rectangle)
            elif ee_designator == "TEXT":
                ee_text = ee_footprint_text(**dict(zip(ee_footprint_text.__fields__, ee_fields)))
