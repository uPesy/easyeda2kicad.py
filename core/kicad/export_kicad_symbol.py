...
class exporter_symbol_kicad():
    ...
    def generate_kicad_symbol(self):
        ...
        for ee_pin in self.input.pins:
            ...
            ki_pin.style += KI_PIN_STYLES['clock'] if ee_pin.clock.is_displayed == '1' else ''
        ...
        for ee_polyline in self.input.polylines:
            ...
            x_points = [ resize_to_kicad(int(float(raw_pts[i])) - int(self.input.bbox.x)) for i in range(0,len(raw_pts),2)]
            y_points = [ resize_to_kicad(int(float(raw_pts[i])) - int(self.input.bbox.y)) for i in range(1,len(raw_pts),2)]

            ki_polyline = ki_symbol_polyline(
                points=[[str(x_points[i]),str(y_points[i])] for i in range(min(len(x_points), len(y_points)))],
